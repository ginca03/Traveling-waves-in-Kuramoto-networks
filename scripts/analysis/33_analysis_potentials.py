...
This script is used to analyze the simulation results of a Jansen-Rit cortical network model oscillating at around 10 Hz.
...
## load activity
activity = np.load(os.path.join(data_path, f"{experiment_id}_simulations/{experiment_id}_simulation_{simulation_id}.npy"))
...
np.save(os.path.join(data_path, f"{experiment_id}_analysis_potentials", f"{experiment_id}f_potential_div_lh_{simulation_id}.npy"), U_lh)
np.save(os.path.join(data_path, f"{experiment_id}_analysis_potentials", f"{experiment_id}f_potential_div_rh_{simulation_id}.npy"), U_rh)