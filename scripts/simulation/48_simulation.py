...
via the frequency-specific structural connectome inferred from HCP subjects using nonnegative matrix factorization. 
The parameters explored are the intrinsic frequency, global coupling strength, and conduction speed.

Usage
-----
    python 48_simulation.py "configuration/48_configuration.yaml" process_id
...
simulation_idx, frequency_idx, coupling_strength_idx, conduction_speed_idx = list(itertools.product(*[range(0,10), range(1,3), range(0,39), range(0,10)]))[process_id]
...
band = band_names[frequency_idx]
weights = np.load(os.path.join(data_path, weights_path + f"{band}.npy"))
