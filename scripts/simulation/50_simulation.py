#!/usr/bin/env python
""" Cortical network model based on Schaefer parcellation - CONTROL with Noise
...
this control simulation we add gaussian noise to the model.

Usage
-----
    python 50_simulation.py "configuration/your_configuration_file.yaml" simulation_idx
...
# ----------------
# model parameters
noise_sigma = np.array([float(config["noise_sigma"])])  # additive noise dispersion
...
sim = simulator.Simulator(model = model, 
                          ...
                          integrator = integrators.HeunStochastic(dt=integration_step_size, noise=noise.Additive(nsig=noise_sigma)),
...
# save data
np.save(os.path.join(data_path, f"50_simulations/50_simulation_{simulation_id}.npy"), phase)