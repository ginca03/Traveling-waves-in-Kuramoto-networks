...
This script is used to simulate a control cortical network model consisting of Kuramoto oscillators that are connected
via the permuted structural connectome inferred from HCP subjects based on the Schaefer et al. (2018) parcellation.

Usage
-----
    python 31_simulation.py "configuration/your_configuration_file.yaml" simulation_idx
...
# Permute Structural Connectivity
# -------------------------------
# permute structural connectivity weights while preserving topology
# get upper triangular weight matrix
weights_triu = np.triu(weights)
# randomly permute weights
weights_triu[weights_triu!=0] = np.random.permutation(weights_triu[weights_triu!=0])
# make matrices symmetric
weights = weights_triu + weights_triu.T - np.diag(np.diag(weights_triu))
...
# save data
np.save(os.path.join(data_path, f"31_simulations/31_simulation_{simulation_id}.npy"), phase)