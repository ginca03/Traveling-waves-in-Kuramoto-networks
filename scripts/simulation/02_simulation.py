#!/usr/bin/env python
""" Travelling waves follow a connectivity instrength gradient - CONTROL

This script is used to simulate a 2D network of Kuramoto oscillators. This script produces
control simulations without an instrength gradient.

Usage
-----
    python 02_simulation.py "configuration/your_configuration_file.yaml" simulation_idx
...
# create spatially uniform in-strength
instrength_gradient_offset = float(config["instrength_gradient_offset"])
instrength_gradient_field = np.zeros(N) + instrength_gradient_offset

# create connection strength distribution based on in-strength gradient field
sc.weights = (A * instrength_gradient_field).T
assert np.all(np.isclose(sc.weights.sum(axis=1)[0], sc.weights.sum(axis=1)))  # check that instrength is homogenous
...
# save data
# save data
os.makedirs(os.path.join(data_path, "02_simulations"), exist_ok=True)
np.save(os.path.join(data_path, f"02_simulations/02_simulation_{simulation_id}.npy"), phase)
