#!/usr/bin/env python
""" Cortical network model based on Schaefer parcellation - CONTROL with frequency dispersion

This script is used to simulate a cortical network model consisting of Kuramoto oscillators that are connected
via the structural connectome inferred from HCP subjects based on the Schaefer et al. (2018) parcellation. In
this control simulation we add gaussian intrinsic frequency dispersion to the model.

Usage
-----
    python 51_simulation.py "configuration/your_configuration_file.yaml" simulation_idx
...
intrinsic_frequency_distribution = np.random.normal(0, intrinsic_frequency_standard_deviation, size=N) + intrinsic_frequency_mean
...
# save data
np.save(os.path.join(data_path, f"{experiment_id}_simulations/{experiment_id}_simulation_{simulation_id}.npy"), phase)