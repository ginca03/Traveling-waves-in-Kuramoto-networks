...
This script is used to simulate a full brain network model consisting of Kuramoto oscillators that are connected
via the original structural connectome inferred from HCP subjects based on the Schaefer et al. (2018) parcellation
but with constant delay corresponding to the average delay based on the structural connectome (~23 ms).

Usage
-----
    python 39_simulation.py "configuration/your_configuration_file.yaml" simulation_idx
...
# ---------------------
# This ensures an average time delay between regions
lengths[lengths!=0] = np.mean(lengths[lengths!=0])
...
# save data
np.save(os.path.join(data_path, f"39_simulations/39_simulation_{simulation_id}.npy"), phase)